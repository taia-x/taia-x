...
let buy(buy_parameters, storage : buy_param * taia_x_storage) : (operation  list) * taia_x_storage =
    ...
    then (failwith("A dataset can only be bought by a consumer") : (operation  list) * taia_x_storage)
      ...
      in
      // check if dataset cert is valid
      let cert_valid : bool = check_cert_valid(buy_parameters.token_id, storage) in
      if(cert_valid) then
        ...
        (failwith("The dataset certificate is invalid") : (operation  list) * taia_x_storage)