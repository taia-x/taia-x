...
(**
Put the dataset on sale in the "on_sale" list and add this contract as an operator for this token
...
let sell (sell_params, storage : sell_param * nft_token_storage) : (operation  list) * nft_token_storage =
    ...
    then (failwith("Only the owner of a dataset can sell it") : (operation  list) * nft_token_storage)
    else
        let dataset : dataset = get_dataset_from_datasets(sell_params.token_id, storage.market.datasets) in
            if dataset.onSale then
                (failwith("This dataset is already on sale"): (operation  list) * nft_token_storage)
            else
                ...
                let datasets_with_updated_dataset: datasets =  update_price_on_sale_flag_for_a_dataset(sell_params.token_id, true, sell_params.price, storage.market.datasets) in
                ([] : operation list), { storage with market = { storage.market with sales = sales_with_new_sale; datasets = datasets_with_updated_dataset }; operators = operators_with_contract_as_operator }