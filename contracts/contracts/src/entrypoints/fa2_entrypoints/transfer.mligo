...
let fa2_transfer (_transfers_list, storage: transfer list * nft_token_storage): (operation  list) * nft_token_storage =
    let _validator = default_operator_validator in
    ...
    let new_storage = { storage with ledger = storage.ledger; market = { storage.market with owners = storage.market.owners; datasets=storage.market.datasets}; }
    in
