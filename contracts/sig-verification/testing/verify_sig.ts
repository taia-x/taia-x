...
async function verify_sig() {
    ...
    const signer: any = new InMemorySigner('edsk3JRacRgUiZY7ciHsqRbPEqBcHASQpvoHoUAvmB1uq7Fu5RHzam');
        const tezos = new TezosToolkit(provider);
    tezos.setSignerProvider( signer );
    try {
        console.log("signer pkh:");
        console.log(await signer.publicKeyHash());
        const contract = await tezos.contract.at('KT1QS8m9TzQF3PvJzxpDgqHYGTtBWqW3FCev');
