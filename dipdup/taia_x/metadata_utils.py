...
async def fetch_metadata_cf_ipfs(token, failed_attempt=0):
    ...
    try:
        session = aiohttp.ClientSession()
        data = await http_request(session, 'get', url=f'http://ipfs:8080/ipfs/{addr}', timeout=10) if os.environ['IPFS_HOST'] else http_request(session, 'get', url=f'http://127.0.0.1:8081/ipfs/{addr}', timeout=10)
        print(data)
        await session.close()
